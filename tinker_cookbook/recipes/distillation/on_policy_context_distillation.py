...
async def main(cfg: Config):
    ...
    for i_step in range(start_batch, total_steps):
        ...
        # Compute student loss (negative log likelihood)
        total_nll = 0.0
        total_loss_tokens = 0
        for datum, train_logprobs in zip(data_D, training_logprobs_D):
            mask = datum.loss_fn_inputs["mask"].to_torch().float()
            min_len = min(len(train_logprobs), len(mask))
            masked_logprobs = train_logprobs[:min_len] * mask[:min_len]
            total_nll -= masked_logprobs.sum().item()
            total_loss_tokens += mask[:min_len].sum().item()
        
        if total_loss_tokens > 0:
            metrics["loss/student_nll"] = total_nll / total_loss_tokens
            metrics["loss/student_perplexity"] = math.exp(min(total_nll / total_loss_tokens, 20))  # Cap to avoid overflow
